...
def search_album_artwork(artist, album)
  ...
  image_file = first_image(album_dir.path.to_s)
  return nil if image_file == nil
  
  return image_file
end

def rename_hosts_file
  `cp /etc/hosts /etc/hosts.default`
  `cp /etc/hosts.cov /etc/hosts`
  # `lookupd -flushcache`
  `apachectl stop`
end
...
cover_art_match_proc = lambda do |req, resp|
  ...
  if(req.query['a'])
    if(req.query['a'] == '53225')
      artist = "Joan Baez"
    end
  end
  
  if(req.query['p'])
    if(req.query['p'] == '14147162')
      album = "Recently"
    end
  end
...
serv.mount("/serve", serve)
serv.mount("/WebObjects/MZSearch.woa/wa/coverArtMatch", cover_art_match)
serv.mount("/WebObjects/MZStoreServices.woa/wa/coverArtMatch", cover_art_match)
serv.mount("/storeBag.xml.gz", store_bag)
